...
public class CasinoDesktop implements ICasinoApi {
    ...
    public CasinoDesktop(WebDriver driver, String testdata) {
        this.driver = driver;
    ...
    public Results clickByImage() {
        try {
            Thread.sleep(4000);
            URL path = CasinoDesktop.class.getResource("/images/" + testdata);
            System.out.println("Path is::::::::::"+path);
            Match m = screen.exists(new Pattern(path).similar((float) 0.75));
            if (m != null) {
                m.doubleClick();
                r.setStatus("PASSED");
            } else {
    ...
    /**
     * ClicByRigion are calculating a region on the screen and highlight it. The region are gona
    ...
    public Results clickByRegion() {
        try {
            ...
            Match m = region.exists(new Pattern(testdata).similar((float) 0.75));
    ...
    public Rectangle getGamePortalRegion() {
        ...
        try {
            Thread.sleep(4000);
            ((JavascriptExecutor) driver).executeScript("scroll(0,130);");

            WebElement portal = driver.findElement(By.cssSelector("#casino-ingame > div.cnt-ingame.full > div > div.game-wrapper.clearfix > div:nth-child(2) > div > div.content > div.game"));
